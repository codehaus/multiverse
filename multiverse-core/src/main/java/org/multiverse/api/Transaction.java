...
/**
 * All changes on transaction objects must be done through a Transaction. The transaction make sure that the
 * changes are:
 * <ol>
 * <li>Atomic: all or nothing gets committed (Failure atomicity)</li>
 * <li>Consistent : </li>
 * <li>Isolated: a transaction
 * is executed isolated from other transactions. Meaning that a transaction won't see changed made by transactions
 * executed concurrently, but it will see changes made by transaction completed before.</li>
 * </ol>
 * <p/>
 * <h3>Thread-safety</h3>
 * A Transaction is not thread-safe to use (just like a Hibernate Session is not thread-safe to
 * use). It can be handed over from thread to thread, but one needs to be really careful with threadlocals.
 * Although the Stm/Transaction implementation don't care about threadlocals, the stuff in front (templates,
 * instrumented code etc) could depend on threadlocals.
 * <p/>
 * <h3>Listen to lifecycles</h3>
 * It is possible to listen to a Transaction when it aborts, or commits. This can be done
 ...
 * <h3>Blocking</h3>
 * It is possible to let a transaction block until some state changes has happened; comparable with the
 * waitset/{@link java.util.concurrent.locks.Condition} functionality already provided. For more information see the
...
public interface Transaction {
    ...
    void join(TwoPhaseCommitGroup group);

    /**
     * Registers the retry Latch. This functionality is required for the retry mechanism (so blocking!) and is
     ...
     * <p/>
     * If the execution of one of the listeners fails, the others won't be executed. If it listeners fails
     * before the precommit, the transaction is aborted (no matter what).
     * <p/>
     ...
     * This is because it is not running in the same transaction because the current transaction already is committed
     * or aborted.
     * <p/>
